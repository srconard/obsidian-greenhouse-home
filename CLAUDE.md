...
- Location (v1.3.0, 2026-09-13): `settings.location` = `main` (default) | `left` | `right`, chosen from the ⚙ menu ("Open in…"), the settings tab, or the three `open-greenhouse-<loc>` commands; `plugin.moveTo(loc)` saves and re-opens. `activateView()` reuses a greenhouse leaf only if it is already on the chosen side (`leaf.getRoot()` vs `workspace.leftSplit`/`rightSplit`) — a leaf cannot be moved across splits, so a wrong-side one is detached and a new one made with `getLeftLeaf(false)` / `getRightLeaf(false)` / `getLeaf(true)`. Width classes `gh-narrow` (<600px) / `gh-tight` (<380px) are set from the view's own width in the **public `onResize()` hook** (a deliberate override of the View lifecycle hook — the reserved-names rule below is about *private helpers* shadowing lifecycle methods, not about overriding the hook itself) and re-applied in `render()`; a media query would read the window, and the phone drawer is ~350px wide.
